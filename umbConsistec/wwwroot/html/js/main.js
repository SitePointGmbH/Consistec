            ...
            if ($(window).width() > 1199) {
                ...
                if (stackItems.length) {
                    ...
                    for (let i = 0; i < stackItems.length - 1; i++) {
                        ...
                        var stackTopPosition = _this.offset().top + 150;
...
const zoomVideo = document.querySelector('.zoom-scroll-video');
const section = document.querySelector('#adler-section');
...
window.addEventListener('scroll', () => {
    ...
    if (rect.top < windowHeight && rect.bottom > 0) {
        // Progress: 0 when section enters, 1 when scrolled away
        const progress = Math.max(0, Math.min(1, 1 - (rect.bottom / windowHeight)));

        // Starts big (1.0), shrinks to 0.75 as user scrolls down
        const scale = 1.0 - (progress * 0.25);

        ecotelSection.style.setProperty(
            '--parallax-transform',
            `scale(${scale})`
        );
    }
